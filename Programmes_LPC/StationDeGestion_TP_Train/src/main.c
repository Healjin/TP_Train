...
/**
 * integer to string in the base desired
...
/**
 * Interruption generated by pressing on the touchscreen OR the rotate button
 * @brief When someone press on the screen this interruption is called,
 * @brief we run a timer to ignore all rebounds. The "flag_interrupt", which
 * @brief valid the pressure, is only set after the end of timer. This flag
 * @brief is scrutinized in the main function.
 * @brief If we press the rotate button we just need to see if the other "edge"
 * @brief state to know which way we are turning.
 */
...
/**
 * Interrupt generated by a timer when we press on the touchscreen
 * @brief This timer permit to ignore multiple pressures on the screen
 * @brief because of rebounds.
 */
...
/**
 * Main program, initialize all devices and control the touchscreen
 * pressures. If speed, light or direction if changed on the screen
 * the program will send a frame through the UART that is connected
 * on the XBEE.
 * If someone change something on the station Marklin, all datas are
 * actualized on station (ExtLab2) screen.
 */
int main(void) {
